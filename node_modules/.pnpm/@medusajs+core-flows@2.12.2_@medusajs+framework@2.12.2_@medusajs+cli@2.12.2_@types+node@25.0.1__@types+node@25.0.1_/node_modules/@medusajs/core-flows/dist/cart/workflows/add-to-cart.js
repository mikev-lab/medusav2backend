...
const cartFields = ["completed_at", "locale"].concat(fields_1.cartFieldsForPricingContext);
...
exports.addToCartWorkflow = (0, workflows_sdk_1.createWorkflow)({
    ...
    const { variants: variantsData, lineItems: lineItemsData } = (0, workflows_sdk_1.when)("should-calculate-prices", { variantIds }, ({ variantIds }) => {
        return !!variantIds.length;
    }).then(() => {
        const { variants: variantsData, lineItems: items } = get_variants_and_items_with_prices_1.getVariantsAndItemsWithPrices.runAsStep({
            input: {
                cart,
                items: input.items,
        ...
        const lineItems = (0, workflows_sdk_1.transform)({ items }, ({ items }) => {
    ...
    const fetchedVariants = (0, workflows_sdk_1.when)("fetch-variants", { variantsData, variantIds }, ({ variantsData, variantIds }) => {
        return !variantsData?.length && !!variantIds.length;
    }).then(() => {
        return (0, common_1.useQueryGraphStep)({
            entity: "variants",
            fields: (0, utils_1.deduplicate)([
                ...fields_1.productVariantsFields,
                ...prepare_confirm_inventory_input_1.requiredVariantFieldsForInventoryConfirmation,
    ...
    const variants = (0, workflows_sdk_1.transform)({ variantsData, fetchedVariants }, ({ variantsData, fetchedVariants }) => {
        return (variantsData ??
            fetchedVariants);
    });
    const lineItems = (0, workflows_sdk_1.transform)({ cart_id: input.cart_id, items: input.items, lineItemsData, variants }, ({ cart_id, items: items_, lineItemsData, variants }) => {
    ...
    const itemsToCreateVariants = (0, workflows_sdk_1.transform)({ itemsToCreate, variants }, (data) => {
    ...
    const translatedItemsToCreate = (0, steps_1.getTranslatedLineItemsStep)({
        items: itemsToCreate,
