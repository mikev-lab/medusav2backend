...
exports.exchangeRequestItemReturnValidationStep = (0, workflows_sdk_1.createStep)("exchange-request-item-return-validation", async function ({ order, orderChange, orderReturn, orderExchange, items, }) {
    ...
    (0, order_validation_1.throwIfManagedItemsNotStockedAtReturnLocation)({
        order,
        orderReturn,
        inputItems: items,
    });
...
exports.orderExchangeRequestItemReturnWorkflow = (0, workflows_sdk_1.createWorkflow)(exports.orderExchangeRequestItemReturnWorkflowId, function (input) {
    ...
    const pickItemLocationId = (0, workflows_sdk_1.transform)({ order, input }, ({ order, input }) => {
        if (input.location_id) {
            return input.location_id;
        }
        // pick the first item location
        const item = order?.items?.find((item) => item.id === input.items[0].id);
        let locationId;
        (0, utils_1.deepFlatMap)(item, "variant.inventory_items.inventory.location_levels", ({ location_levels }) => {
            if (!locationId && (0, utils_1.isDefined)(location_levels?.location_id)) {
