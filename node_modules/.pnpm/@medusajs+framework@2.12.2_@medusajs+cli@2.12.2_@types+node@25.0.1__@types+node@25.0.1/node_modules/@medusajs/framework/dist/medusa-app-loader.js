...
class MedusaAppLoader {
     ...
     * @param action
     */
    async runModulesMigrations(options = {
        action: "run",
    }) {
        ...
        if (options.action === "revert") {
            await (0, modules_sdk_1.MedusaAppMigrateDown)(options.moduleNames, migrationOptions);
        }
        else if (options.action === "run") {
            await (0, modules_sdk_1.MedusaAppMigrateUp)(migrationOptions);
        }
        else if (options.action === "generate") {
            await (0, modules_sdk_1.MedusaAppMigrateGenerate)(options.moduleNames, migrationOptions);
        }
    }
    /**
     ...
     */
    async reloadSingleModule({ moduleKey, serviceName, }) {
        ...
        modules_sdk_1.MedusaModule.unregisterModuleResolution(moduleKey);
        if (serviceName) {
            __classPrivateFieldGet(this, _MedusaAppLoader_container, "f").cache.delete(serviceName);
        }
        ...
        const result = await (0, modules_sdk_1.MedusaApp)({
        ...
        const loadedModule = result.modules[moduleKey];
        if (loadedModule) {
            __classPrivateFieldGet(this, _MedusaAppLoader_container, "f").register({
                [loadedModule.__definition.key]: (0, awilix_1.asValue)(loadedModule),
            });
        }
        if (loadedModule?.__hooks?.onApplicationStart) {
            await loadedModule.__hooks.onApplicationStart
                .bind(loadedModule)()
                .catch((error) => {
                injectedDependencies[utils_1.ContainerRegistrationKeys.LOGGER].error(`Error starting module "${loadedModule.__definition.key}": ${error.message}`);
            });
        }
        return loadedModule;
    }
